...
#' @export
#' @return Data frame
pathwas_ld_clump <- function(dat=NULL, clump_kb=10000, clump_r2=0.001, clump_p=0.99,
                     pop = "EUR", access_token=NULL, bfile=NULL, plink_bin=NULL, def_tmpDir=tempdir())
{
  ...
  for(i in 1:length(ids))
  {
    ...
    if(nrow(x) == 1)
    {
      ...
      if(is.null(bfile))
      {
        ...
        res[[i]] <- pathwas_ld_clump_local(x, clump_kb=clump_kb, clump_r2=clump_r2, clump_p=clump_p, bfile=bfile, plink_bin=plink_bin, def_tmpDir=def_tmpDir)
      }
    }
  }
